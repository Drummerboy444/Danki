...
for(var i=0; i<ds_list_size(list_effectsBuffer); i++){
	...
	var _enum_effect = EffectData_getEffect(_EffectData_buffer);
	var _id_caster = EffectData_getCasterId(_EffectData_buffer);
	...
	for (var j = ds_list_size(list_activeEffects) - 1; j >= 0; j--) {
		...
		var _enum_activeEffect = EffectData_getEffect(_EffectData_active);
		var _id_activeCaster = EffectData_getCasterId(_EffectData_active);
